...
const isSerialHost = (host: Host) => (host.protocol ?? "ssh") === "serial";
const isNetworkHost = (host: Host) => !isSerialHost(host);
const displayStatusLabel = (host: Host) => isSerialHost(host) ? "Local serial" : statusLabel[host.status];
const displayStatusClass = (host: Host) => isSerialHost(host) ? "serial" : host.status;
const credentialAutoLogin = (profile?: CredentialProfile) => profile?.autoLogin !== false;
...
const defaultPlatforms = ["Cisco IOS", "Cisco IOS-XE", "Cisco NX-OS", "Arista EOS", "Juniper JunOS", "Palo Alto", "Fortinet FortiOS", "Linux", "Other"];
...
async function saveTerminalTranscript(session: Session) {
  ...
  const fileName = `${safeFileName(session.host.name)}-${date}.txt`;
  if (isTauri()) {
    const selected = await saveFileDialog({ defaultPath: fileName, filters: [{ name: "Text transcript", extensions: ["txt"] }] });
...
function App() {
  ...
  const importDeviceSessions = (imported: ImportedSession[], source: string, assignedCredentialId?: string) => {
    ...
    for (const item of imported) {
      ...
      let credentialId: string | undefined = item.protocol === "serial" ? undefined : assignedCredentialId;
      if (!credentialId && item.protocol !== "serial" && item.username) {
        ...
        if (!profile) {
          profile = { id: `credential-${crypto.randomUUID()}`, label: item.credentialLabel ?? `${source} · ${item.username}`, username: item.username, autoLogin: false };
  ...
  const completeWelcomeTour = async (result: WelcomeTourResult) => {
    const credential = result.credential;
    if (credential) {
      try {
        if (credential.password) await saveCredentialPassword(credential.profile.id, credential.password);
        if (credential.enablePassword) await saveCredentialEnablePassword(credential.profile.id, credential.enablePassword);
      } catch (caught) {
        await deleteCredentialEnablePassword(credential.profile.id).catch(() => undefined);
        await deleteCredentialPassword(credential.profile.id).catch(() => undefined);
        throw caught;
      }
      setCredentialProfiles((current) => current.some((profile) => profile.id === credential.profile.id)
        ? current.map((profile) => profile.id === credential.profile.id ? credential.profile : profile)
        : [credential.profile, ...current]);
    }
    setUserProfile(result.profile);
    setPreferences((current) => ({ ...current, sites: result.sites, platforms: result.platforms }));
    if (result.importDemoData) {
      setDeviceHosts((current) => {
        const existingIds = new Set(current.map((host) => host.id));
        const additions = ciscoDemoHosts.filter((host) => !existingIds.has(host.id));
        return additions.length ? [...additions, ...current] : current;
      });
      setView("workspace");
    }
    setOnboardingOpen(false);
    notify(`Welcome to NetSSH${result.profile.name ? `, ${result.profile.name.split(" ")[0]}` : ""}`);
  };
  ...
  const startNativeSession = async (id: string, host: Host, username = "", password?: string, reconnecting = false): Promise<string | null> => {
    ...
    const autoLogin = credentialAutoLogin(assignedCredential);
    ...
    try {
      ...
      await startTerminalSession({ sessionId: id, deviceId: host.id, credentialId: autoLogin ? assignedCredential?.id : undefined, autoLogin, protocol, target: host.address, port, baudRate: host.baudRate, username: cleanUsername, password, trustedFingerprint, legacyRsa, legacyKex });
    ...
    <div className={`app-shell ${lightMode ? "theme-light" : "theme-dark"} ${preferences.compactWorkspace ? "compact-workspace" : ""}`}>
      ...
      <main className={`main ${sidebarOpen ? "" : "main-expanded"}`}>
        ...
        <div className="content">
          ...
          }} onAssign={(hostId, credentialId) => setDeviceHosts((current) => current.map((host) => host.id === hostId ? { ...host, credentialId: credentialId || undefined, username: undefined } : host))} onAssignMany={(hostIds, credentialId) => setDeviceHosts((current) => current.map((host) => hostIds.includes(host.id) ? { ...host, credentialId, username: undefined } : host))} />}
        </div>
      </main>
...
function SettingsModal({ preferences, availableUpdate, onUpdateFound, onClose, onSave }: { preferences: AppPreferences; availableUpdate: Update | null; onUpdateFound: (update: Update | null) => void; onClose: () => void; onSave: (preferences: AppPreferences) => void }) {
  ...
  return <div className="modal-backdrop settings-backdrop" onMouseDown={onClose}>
    <section className="settings-modal settings-modal-redesigned" onMouseDown={(event) => event.stopPropagation()}>
      <div className="provider-modal-head settings-header">
        <div><span><Settings size={18} /></span><div><h3>NetSSH settings</h3><p>Personalise your workspace and local device preferences.</p></div></div>
      ...
      <div className="settings-shell">
        <nav className="settings-nav" aria-label="Settings sections">
          ...
          {sections.map((item) => <button type="button" key={item.id} className={section === item.id ? "active" : ""} onClick={() => setSection(item.id)}><item.icon size={16} /><span><strong>{item.label}</strong><small>{item.description}</small></span>{item.id === "updates" && availableUpdate && <i>New</i>}</button>)}
          <div className="settings-nav-note"><ShieldCheck size={15} /><span>Preferences stay on this device.</span></div>
        </nav>
        <div className="settings-content">
          ...
          <div className="settings-scroll">
            ...
            {section === "workspace" && <><div className="settings-preference-grid">
              ...
              <label className="settings-toggle"><span><strong>Compact workspace</strong><small>Reduce tab, toolbar, and terminal spacing</small></span><input type="checkbox" checked={draft.compactWorkspace} onChange={(event) => setDraft({ ...draft, compactWorkspace: event.target.checked })} /></label>
              <label className="settings-toggle"><span><strong>CLI autocomplete</strong><small>Suggest common network commands while typing in terminals</small></span><input type="checkbox" checked={draft.cliAutocomplete} onChange={(event) => setDraft({ ...draft, cliAutocomplete: event.target.checked })} /></label>
              <label className="settings-toggle"><span><strong>Connection safety notices</strong><small>Show authentication and trust limitations in new sessions</small></span><input type="checkbox" checked={draft.showConnectionWarnings} onChange={(event) => setDraft({ ...draft, showConnectionWarnings: event.target.checked })} /></label>
            </div><div className="settings-info-card settings-workspace-note"><Sparkles size={16} /><div><strong>Designed for focused work</strong><p>These preferences change how NetSSH feels day to day. Your device inventory and saved credentials are managed in their dedicated areas.</p></div></div></>}
            {section === "inventory" && <div className="settings-config-stack"><ConfigList title="Inventory sites" description="Available when adding or editing a device" items={draft.sites} placeholder="Add a site" onChange={(sites) => setDraft({ ...draft, sites })} /><ConfigList title="Device platforms" description="Vendor and operating-system choices" items={draft.platforms} placeholder="Add a platform" onChange={(platforms) => setDraft({ ...draft, platforms })} /><div className="settings-info-card"><Server size={16} /><div><strong>Keep lists practical</strong><p>These choices appear in inventory forms and the welcome tour. Add the platforms and sites your team actually uses.</p></div></div></div>}
            {section === "security" && <div className="settings-security-page"><div className="settings-security-grid"><article><span><ShieldCheck size={17} /></span><div><strong>Operating-system vault</strong><p>Credential passwords and enable secrets use the native Windows or macOS vault when available.</p></div></article><article><span><LockKeyhole size={17} /></span><div><strong>Local preferences</strong><p>Workspace settings, sites, and platform names are stored locally on this device.</p></div></article></div><div className="settings-security"><ShieldCheck size={16} /><span>NetSSH does not store passwords in preferences, include them in AI prompts, or upload your local inventory.</span></div></div>}
          </div>
        </div>
      </div>
...
function SessionConnectionBadges({ session, compact = false }: { session: Session; compact?: boolean }) {
  ...
  const reachabilityTone = isSerialHost(session.host) ? "pending" : session.host.status === "online" ? "good" : session.host.status === "warning" ? "pending" : "bad";
...
function Workspace({ sessions, activeId, session, hosts, userName, autocompleteEnabled, onAuthenticate, onReconnect, onActivate, onClose, onCloseMany, onConnect, onNewSession, onCommand, onTerminalData, onAddDevice, onShowInventory, onRefreshReachability, reachabilityRefreshing, reachabilityLastChecked, notify }: { sessions: Session[]; activeId: string | null; session?: Session; hosts: Host[]; userName: string; autocompleteEnabled: boolean; onAuthenticate: (id: string, credentials: ConnectionCredentials) => Promise<void>; onReconnect: (id: string) => Promise<void>; onActivate: (id: string) => void; onClose: (id: string) => void; onCloseMany: (ids: string[]) => void; onConnect: (host: Host) => void; onNewSession: (host: Host) => Promise<string | null>; onCommand: (id: string, lines: TerminalLine[]) => void; onTerminalData: (id: string, data: string) => void; onAddDevice: () => void; onShowInventory: () => void; onRefreshReachability: () => Promise<void>; reachabilityRefreshing: boolean; reachabilityLastChecked: number | null; notify: (message: string) => void }) {
  ...
  const saveLog = async () => {
    try {
      if (await saveTerminalTranscript(primary)) notify("Terminal transcript saved");
    } catch (caught) {
      notify(`Could not save terminal transcript: ${String(caught)}`);
    }
  };
  return (
    <section className="terminal-layout">
      ...
      {layout === "notes" && <div className="workspace-panes engineer-notes-split"><SessionPane session={primary} sessions={sessions} active autocompleteEnabled={autocompleteEnabled} onAuthenticate={onAuthenticate} onReconnect={() => onReconnect(primary.id)} onSelect={(id) => { setPrimaryId(id); onActivate(id); }} onActivate={() => onActivate(primary.id)} onData={(data) => onTerminalData(primary.id, data)} /><div className="workspace-notes-pane"><EngineerNotes notify={notify} /></div></div>}
    ...
    <div className="page workspace-home">
      <div className="workspace-overview">
        <section className="welcome-card">
          ...
          <div className="welcome-devices">{hosts.slice(0, 3).map((host) => <button key={host.id} onClick={() => onConnect(host)}><span className={`device-state ${displayStatusClass(host)}`} /><span><strong>{host.name}</strong><small>{host.address}</small></span><ChevronRight size={14} /></button>)}</div>
        </section>
...
function DeviceCard({ host, onConnect, onFavorite }: { host: Host; onConnect: (host: Host) => void; onFavorite?: (id: string) => void }) {
  return <button className="device-card" onClick={() => onConnect(host)}><div className="device-top"><span className="device-icon"><Router size={20} /></span><Star size={15} role={onFavorite ? "button" : undefined} tabIndex={onFavorite ? 0 : undefined} aria-label={host.favorite ? `Remove ${host.name} from favourites` : `Add ${host.name} to favourites`} className={host.favorite ? "starred" : ""} onClick={(event) => { if (!onFavorite) return; event.stopPropagation(); onFavorite(host.id); }} onKeyDown={(event) => { if (onFavorite && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); event.stopPropagation(); onFavorite(host.id); } }} /></div><strong>{host.name}</strong><code>{host.address}</code><div className="device-meta"><span className={`device-state ${displayStatusClass(host)}`} />{displayStatusLabel(host)}<span>·</span>{isSerialHost(host) ? "Not network checked" : host.latency ? `${host.latency} ms` : "No response"}</div><div className="device-footer"><span>{deviceRoleLabel(host)} · {host.platform} · {(host.protocol ?? "ssh").toUpperCase()}</span><ChevronRight size={15} /></div></button>;
}
...
function InteractiveTerminal({ session, onData, onReconnect, autocompleteEnabled, autoFocus = true }: { session: Session; onData: (data: string) => void; onReconnect: () => Promise<void>; autocompleteEnabled: boolean; autoFocus?: boolean }) {
  ...
  useEffect(() => {
    ...
    const x = Math.max(8, Math.min(contextMenu.x, window.innerWidth - menu.width - 8));
    const y = Math.max(8, Math.min(contextMenu.y, window.innerHeight - menu.height - 8));
...
function Inventory({ hosts, onConnect, onAdd, onTransfer, onEdit, onFavorite, onDelete }: { hosts: Host[]; onConnect: (host: Host) => void; onAdd: () => void; onTransfer: () => void; onEdit: (host: Host) => void; onFavorite: (id: string) => void; onDelete: (id: string) => void }) {
  ...
  const [status, setStatus] = useState<"all" | Host["status"] | "serial">("all");
...
function SessionTransferModal({ hosts, credentialProfiles, configuredSites, onClose, onImport, notify }: { hosts: Host[]; credentialProfiles: CredentialProfile[]; configuredSites: string[]; onClose: () => void; onImport: (sessions: ImportedSession[], source: string, assignedCredentialId?: string) => { added: number; duplicates: number; profiles: number }; notify: (message: string) => void }) {
  ...
  const applyImport = () => {
    ...
    const summary = onImport(sessions, source, importCredentialId || undefined);
...
function Credentials({ profiles, hosts, notify, onSave, onDelete, onAssign, onAssignMany }: { profiles: CredentialProfile[]; hosts: Host[]; notify: (message: string) => void; onSave: (profile: CredentialProfile, password?: string, enablePassword?: string) => Promise<void>; onDelete: (profile: CredentialProfile) => Promise<void>; onAssign: (hostId: string, credentialId: string) => void; onAssignMany: (hostIds: string[], credentialId: string) => void }) {
  ...
  const allSelected = assignableHosts.length > 0 && selectedHostIds.length === assignableHosts.length;
  const selectAllHosts = () => setSelectedHostIds(allSelected ? [] : assignableHosts.map((host) => host.id));
  const toggleHost = (hostId: string) => setSelectedHostIds((current) => current.includes(hostId) ? current.filter((id) => id !== hostId) : [...current, hostId]);
  const applyBulkAssignment = () => {
    if (!bulkCredentialId || !selectedHostIds.length) return;
    onAssignMany(selectedHostIds, bulkCredentialId);
    const profile = profiles.find((item) => item.id === bulkCredentialId);
    notify(`Assigned ${profile?.label ?? "login"} to ${selectedHostIds.length} devices`);
    setSelectedHostIds([]);
  };
  return <div className="page"><div className="page-intro"><div><h2>Credential vault</h2><p>Create a login once, label it clearly, then assign it to any number of devices.</p></div><div className="credential-page-actions"><span className={`native-badge ${isNativeApp() ? "available" : ""}`}><LockKeyhole size={14} />{isNativeApp() ? "Native vault active" : "Native app required"}</span><button className="primary-button" onClick={() => setEditing(null)}><Plus size={16} /> New login</button></div></div>{profiles.length ? <section className="inventory-table credential-table"><div className="credential-row credential-head"><span>Login profile</span><span>Username</span><span>Assigned</span><span>Secrets</span><span /></div>{profiles.map((profile) => { const assigned = hosts.filter((host) => host.credentialId === profile.id).length; const secrets = stored[profile.id]; return <div className="credential-row" key={profile.id}><span><strong>{profile.label}</strong><small>Reusable network login</small></span><span>{profile.username}</span><span>{assigned} device{assigned === 1 ? "" : "s"}</span><span className={secrets?.login ? "vault-stored" : "vault-empty"}><i />{secrets?.login ? secrets.enable ? "Login + enable stored" : "Login password stored" : "Login password required"}</span><span className="row-actions"><button className="connect-button" onClick={() => setEditing(profile)}><Pencil size={13} /> Edit</button><button className="delete-device" aria-label={`Delete ${profile.label}`} onClick={() => setPendingDelete(profile)}><Trash2 size={14} /></button></span></div>; })}</section> : <section className="panel credential-empty"><KeyRound size={27} /><strong>No saved logins yet</strong><span>Create profiles such as “Network Admin”, “Read only”, or “Lab TACACS”.</span><button className="primary-button" onClick={() => setEditing(null)}><Plus size={15} /> Create first login</button></section>}<section className="panel credential-assignments"><div className="panel-title"><div><h3>Device assignments</h3><p>Select devices, choose one saved login, and apply it in one step</p></div></div><div className="assignment-bulk"><div><strong>{selectedHostIds.length} selected</strong><small>Select all devices or choose individual rows.</small></div><button className="secondary-button" type="button" onClick={selectAllHosts}>{allSelected ? "Clear all" : "Select all devices"}</button><select aria-label="Bulk credential assignment" value={bulkCredentialId} onChange={(event) => setBulkCredentialId(event.target.value)}><option value="">Choose saved login…</option>{profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.label} · {profile.username}</option>)}</select><button className="primary-button" type="button" disabled={!selectedHostIds.length || !bulkCredentialId} onClick={applyBulkAssignment}>Assign selected</button></div><div className="assignment-table"><div className="assignment-row assignment-head"><span className="assignment-check"><input type="checkbox" checked={allSelected} onChange={selectAllHosts} aria-label="Select all assignable devices" /></span><span>Device</span><span>Protocol</span><span>Saved login</span></div>{assignableHosts.map((host) => <div className="assignment-row" key={host.id}><span className="assignment-check"><input type="checkbox" checked={selectedHostIds.includes(host.id)} onChange={() => toggleHost(host.id)} aria-label={`Select ${host.name}`} /></span><span><strong>{host.name}</strong><small>{host.address}</small></span><span className="protocol-pill">{(host.protocol ?? "ssh").toUpperCase()}</span><select aria-label={`Login for ${host.name}`} value={host.credentialId ?? ""} onChange={(event) => onAssign(host.id, event.target.value)}><option value="">Ask when connecting</option>{profiles.map((profile) => <option value={profile.id} key={profile.id}>{profile.label} · {profile.username}</option>)}</select></div>)}</div></section>{editing !== undefined && <CredentialProfileModal profile={editing ?? undefined} onClose={() => setEditing(undefined)} onSave={save} />}{pendingDelete && <ConfirmModal title={`Delete ${pendingDelete.label}?`} message="Devices using this login will return to asking for credentials when connecting. Both login and enable passwords are removed from the vault." confirmLabel="Delete login" onCancel={() => setPendingDelete(null)} onConfirm={() => { const profile = pendingDelete; setPendingDelete(null); onDelete(profile).then(() => notify(`${profile.label} deleted`)).catch((caught) => notify((caught as Error).message)); }} />}</div>;
}
...
function CredentialProfileModal({ profile, onClose, onSave }: { profile?: CredentialProfile; onClose: () => void; onSave: (profile: CredentialProfile, password?: string, enablePassword?: string) => Promise<void> }) {
  ...
  const [autoLogin, setAutoLogin] = useState(profile?.autoLogin !== false);
  const [password, setPassword] = useState("");
  ...
  const submit = async (event: FormEvent) => {
    ...
    try {
      await onSave({ id: profile?.id ?? `credential-${crypto.randomUUID()}`, label: label.trim(), username: username.trim(), autoLogin }, password || undefined, enablePassword || undefined);
    } catch (caught) { setError((caught as Error).message); }
  };
  return <div className="modal-backdrop" onMouseDown={onClose}><form className="confirm-modal password-modal credential-profile-modal" onSubmit={submit} onMouseDown={(event) => event.stopPropagation()}><span><KeyRound size={19} /></span><h3>{profile ? `Edit ${profile.label}` : "Create saved login"}</h3><p>Save a username for reuse. Passwords and enable passwords are optional, and you can choose whether the login password is used automatically.</p><label><small>Profile label</small><input autoFocus value={label} onChange={(event) => setLabel(event.target.value)} placeholder="Network Admin" /></label><label><small>Username</small><input value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" placeholder="netadmin" /></label><label className="credential-auto-login"><input type="checkbox" checked={autoLogin} onChange={(event) => setAutoLogin(event.target.checked)} /><span><strong>Auto-login with saved password</strong><small>When off, NetSSH uses this username and asks for the SSH password during each session.</small></span></label><label><small>{profile ? "Replace login password (optional)" : "Login password (optional)"}</small><div className="secret-input"><LockKeyhole size={15} /><input type={visible ? "text" : "password"} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="new-password" placeholder={profile ? "Leave blank to keep existing" : "Leave blank to ask at connection time"} /><button type="button" onClick={() => setVisible(!visible)}>{visible ? <EyeOff size={15} /> : <Eye size={15} />}</button></div></label><label><small>{profile ? "Replace enable password (optional)" : "Enable password (optional)"}</small><div className="secret-input"><ShieldCheck size={15} /><input type={enableVisible ? "text" : "password"} value={enablePassword} onChange={(event) => setEnablePassword(event.target.value)} autoComplete="new-password" placeholder={profile ? "Leave blank to keep existing" : "Optional privileged EXEC secret"} /><button type="button" onClick={() => setEnableVisible(!enableVisible)}>{enableVisible ? <EyeOff size={15} /> : <Eye size={15} />}</button></div></label>{error && <div className="modal-error">{error}</div>}<div className="credential-secret-note"><ShieldCheck size={14} />Passwords stay in the operating-system vault. Existing saved passwords remain available if you turn auto-login back on.</div><div className="password-actions"><button type="button" className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!label.trim() || !username.trim()}>Save login</button></div></form></div>;
}
...
function LiveSwitchAuditPanel({ hosts, credentialProfiles, notify }: { hosts: Host[]; credentialProfiles: CredentialProfile[]; notify: (message: string) => void }) {
  ...
  const exportCsv = async () => {
    if (!audit) return;
    const value = createSwitchAuditCsv(audit);
    const fileName = `${safeFileName(audit.deviceName).toLowerCase()}-port-audit-${new Date(audit.checkedAt).toISOString().slice(0, 10)}.csv`;
    if (isTauri()) {
      const selected = await saveFileDialog({ defaultPath: fileName, filters: [{ name: "CSV spreadsheet", extensions: ["csv"] }] });
      if (typeof selected !== "string") {
        notify("Switch audit export cancelled");
        return;
      }
      await writeTextFile(selected, value);
      notify(`Switch audit saved to ${selected}`);
      return;
    }
    const blobUrl = URL.createObjectURL(new Blob([value], { type: "text/csv;charset=utf-8" }));
    const anchor = document.createElement("a");
